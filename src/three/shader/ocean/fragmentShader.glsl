...
const float SEA_HEIGHT=.2;
const float SEA_CHOPPY=1.;
const float SEA_SPEED=.5;
const float SEA_FREQ=.05;
...
void main(){
    float EPSILON_NRM=.1/iResolution.x;
    ...
    // ray
    vec3 ang=vec3(
        sin(time*3.)*.1,sin(time)*.2+.3,time
    );
    ...
    vec3 light=normalize(vec3(0.0, 1.0, 0.8353));
